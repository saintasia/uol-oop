...
/** retrun vector of all known products in the dataset */
...
/** retrun vector of orders according to the sent filters */
std::vector<OrderBookEntry> OrderBook::getLiveOrders(
  OrderBookType type,
  std::string product,
  ...
  std::vector<OrderBookEntry> orders_sub;
  for (OrderBookEntry& e : orders) {
    for (std::string& timestamp : pastTimeStamps){
      if(e.orderType == type &&
        e.product == product &&
        e.timestamp == timestamp) {
          orders_sub.push_back(e);
...
void OrderBook::removeOrder(OrderBookEntry& order){
  for (int i = 0; i < orders.size(); ++i){
    if (orders[i].price == order.price &&
      ...
      orders[i].username == order.username &&
  ...
  // for bid in bids:
    ...
    // if bid.price >= ask.price # we have a match
      if (bid.price >= ask.price){
        ...
        if (bid.username == "simuser" || bid.username == "botuser"){
          sale.username = bid.username;
          sale.orderType = OrderBookType::bidsale;
        }
        if (ask.username == "simuser" || ask.username == "botuser"){
